...
def identification_genes_for_predictive_model(DSeq2_results_DvsA, DSeq2_results_AvsC, DSeq2_results_DvsC, 
    ...
    DSeq2_results_psvsdc['Gene'] = DSeq2_results_psvsdc['Gene'].apply(lambda x: x.upper())
    DSeq2_results_mtxvsdc['Gene'] = DSeq2_results_mtxvsdc['Gene'].apply(lambda x: x.upper())
    ...
    deg_AvsC = deg_AvsC[deg_AvsC['Gene'].isin(deg_DvsA_fc2['Gene'].values)]
    ...
    # 4
    unidirectional_up = deg_AvsC_up[deg_AvsC_up['Gene'].isin(common_deg_ps_mtx_up['Gene'])]
    unidirectional_up = unidirectional_up.merge(DSeq2_results_DvsC, left_on='Gene', right_on='Gene', suffixes=('_A', '_D'))
    ...
    for k in range(0, len(unidirectional_up)):
        if (unidirectional_up['padj_D'][k] > 0.05) or (abs(unidirectional_up['log2FoldChange_D'][k]) < abs(unidirectional_up['log2FoldChange_A'][k])):
            unidirectional_up_genes.append(unidirectional_up['Gene'][k])

     # 5
    unidirectional_down = deg_AvsC_down[deg_AvsC_down['Gene'].isin(common_deg_ps_mtx_down['Gene'])]
    unidirectional_down = unidirectional_down.merge(DSeq2_results_DvsC, left_on='Gene', right_on='Gene', suffixes=('_A', '_D'))
    ...
    for k in range(0, len(unidirectional_down)):
        if (unidirectional_down['padj_D'][k] > 0.05) or (abs(unidirectional_down['log2FoldChange_D'][k]) < abs(unidirectional_down['log2FoldChange_A'][k])):
            unidirectional_down_genes.append(unidirectional_down['Gene'][k])
...
def take_genes_and_transposition(data, genes):
    ...
    for gene in genes:
        data_trans[gene] = data[data['Gene'] == gene].values[0][1:]
    return data_trans
